...
if ($TipoRol == "ADMINISTRADOR" || $TipoRol == "SUPERVISOR") {
    $totalQuery = "SELECT COUNT(*) as total FROM vw_personal_ingreso WHERE 1=1 $whereSQL";
    ...
    $dataQuery = "SELECT * FROM vw_personal_ingreso WHERE 1=1 $whereSQL ORDER BY $orderColumn $orderDir LIMIT ?, ?";
    ...
    $totalQuery = "SELECT COUNT(*) as total FROM vw_personal_ingreso WHERE id_sede_pl = ? $whereSQL";
    $totalStmt = $Con->prepare($totalQuery);
    ...
    $dataQuery = "SELECT * FROM vw_personal_ingreso WHERE id_sede_pl = ? $whereSQL ORDER BY $orderColumn $orderDir LIMIT ?, ?";
    $dataStmt = $Con->prepare($dataQuery);
