...
if (!$ano || !$semana) {
    $hoy = new DateTime();
    $diaSemana = (int)$hoy->format('N'); // 1=Lunes, 2=Martes, ..., 7=Domingo

    // Ajustar lunes y martes a la semana anterior
    if ($diaSemana <= 2) { // lunes o martes
        $hoy->modify('-2 days'); // lunes->sábado anterior, martes->domingo anterior
    }

    $ano = $hoy->format('o');      // año ISO
    $semana = $hoy->format('W');   // semana ISO
}

// ================================
// 3. Calcular rango de la semana (miércoles→martes)
// ================================
$dto = new DateTime();
$dto->setISODate($ano, $semana, 3); // 3 = miércoles de esa semana
$inicioSemana = $dto->format('Y-m-d'); // miércoles
$dto->modify('+6 days');               // martes siguiente
