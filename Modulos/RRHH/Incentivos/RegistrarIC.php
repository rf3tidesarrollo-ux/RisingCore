    ...
    if (isset($_POST['Insertar'])) {
        ...
        if ($Correcto==7) {
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                do {
                    $stmt = $Con->prepare("INSERT INTO rh_incentivos (id_sede_i, id_personal_i, id_incentivo_i, fecha_i, fecha_inc, hora_inc, cantidad, motivo, id_user_i, estado_i) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)");
                    $stmt->bind_param('iiisssdsi', $SedeVal, $idPersonal, $Incentivo, $FechaVal, $FechaR, $HoraR, $CantidadVal, $Motivo, $ID);
                    $stmt->execute();
                    $stmt->close();
                ...
                session_start();
                $_SESSION['correcto'] = "Incentivo registrado";
